...
export const generateReceipt = (order, customer, type = 'order') => {
  ...
  // Format numbers with commas and N symbol
  const productRows = order.products.map(product => [
    product.name,
    product.quantity.toString(),
    'N' + formatNumber(product.unitPrice),
    'N' + formatNumber(product.totalPrice)
  ]);
  ...
  // Total Amount - properly formatted with commas
  const totalAmountText = 'N' + formatNumber(order.totalAmount);
  doc.setFontSize(11);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(0, 0, 0);
  doc.text('Total Amount:', 135, finalY);
  doc.text(totalAmountText, 195, finalY, { align: 'right' });
  ...
  // Payment Summary Box - wider to accommodate larger numbers with commas
  doc.setDrawColor(200, 200, 200);
  doc.setFillColor(250, 250, 250);
  doc.roundedRect(112, finalY - 5, 83, 25, 2, 2, 'FD');
  
  // Total Paid
  const totalPaidText = 'N' + formatNumber(order.amountPaid);
  doc.setFontSize(10);
  doc.setFont(undefined, 'normal');
  doc.setTextColor(0, 0, 0);
  doc.text('Total Paid:', 117, finalY + 2);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(0, 128, 0);
  doc.text(totalPaidText, 190, finalY + 2, { align: 'right' });
  
  // Balance
  const balanceText = 'N' + formatNumber(order.balance);
  finalY += 8;
  doc.setFontSize(12);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(0, 0, 0);
  doc.text('BALANCE:', 117, finalY + 2);
  doc.setFontSize(13);
  doc.setFont(undefined, 'bold');
  ...
  doc.text(balanceText, 190, finalY + 2, { align: 'right' });
