...
export const generateReceipt = (order, customer, type = 'order') => {
  ...
  // Format numbers without commas to fit better, add N symbol
  const productRows = order.products.map(product => [
    product.name,
    product.quantity.toString(),
    'N' + product.unitPrice.toFixed(2),
    'N' + product.totalPrice.toFixed(2)
  ]);
  ...
  // Total Amount - properly formatted
  const totalAmountText = 'N' + order.totalAmount.toFixed(2);
  doc.setFontSize(11);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(0, 0, 0);
  ...
  // Payment Summary Box - wider to accommodate larger numbers
  doc.setDrawColor(200, 200, 200);
  doc.setFillColor(250, 250, 250);
  doc.roundedRect(115, finalY - 5, 80, 25, 2, 2, 'FD');
  
  // Total Paid
  const totalPaidText = 'N' + order.amountPaid.toFixed(2);
  doc.setFontSize(10);
  doc.setFont(undefined, 'normal');
  doc.setTextColor(0, 0, 0);
  doc.text('Total Paid:', 120, finalY + 2);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(0, 128, 0);
  doc.text(totalPaidText, 190, finalY + 2, { align: 'right' });
  
  // Balance
  const balanceText = 'N' + order.balance.toFixed(2);
  finalY += 8;
  doc.setFontSize(12);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(0, 0, 0);
  doc.text('BALANCE:', 120, finalY + 2);
  doc.setFontSize(13);
  doc.setFont(undefined, 'bold');
