...
export const generateCustomerReceipt = (customer, orders) => {
  ...
  doc.roundedRect(112, yPos - 5, 83, 35, 2, 2, 'FD');
  
  // Total Orders
  doc.setFontSize(10);
  doc.setFont(undefined, 'normal');
  doc.setTextColor(0, 0, 0);
  doc.text('Total Orders:', 117, yPos + 2);
  doc.setFont(undefined, 'bold');
  doc.text('N' + formatNumber(totalOrders), 190, yPos + 2, { align: 'right' });
  
  // Total Paid
  yPos += 8;
  doc.setFont(undefined, 'normal');
  doc.text('Total Paid:', 117, yPos + 2);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(0, 128, 0);
  doc.text('N' + formatNumber(totalPaid), 190, yPos + 2, { align: 'right' });
  ...
  doc.text('N' + formatNumber(balance), 190, yPos + 2, { align: 'right' });
