...
export const generateCustomerReceipt = (customer, orders, options = {}) => {
  ...
  // Period label (shown below title when filtered)
  if (isFiltered) {
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
  ...
  // Old Balance Section (if exists and not a filtered/period statement)
  ...
  if (isFiltered) boxHeight += 8; // extra row for "Period" label
  ...
  // Period label in summary (if filtered)
  if (isFiltered) {
    doc.setFontSize(8);
