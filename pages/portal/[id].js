            ...
            <tbody className="divide-y divide-gray-100">
              {order.products.map((p, i) => (
                <tr key={i}>
                  ...
                  <td className="py-2 text-right font-medium text-gray-800">{fmt(p.totalPrice)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-gray-200">
                <td colSpan={3} className="pt-2 text-right font-semibold text-gray-700">Order Total</td>
                <td className="pt-2 text-right font-bold text-gray-900">{fmt(order.totalAmount)}</td>
              </tr>
              {order.amountPaid > 0 && (
                <tr>
                  <td colSpan={3} className="text-right text-gray-500">Amount Paid</td>
                  <td className="text-right text-green-600 font-medium">{fmt(order.amountPaid)}</td>
                </tr>
              )}
              {order.balance > 0 && (
                <tr>
                  <td colSpan={3} className="text-right text-gray-500">Balance Due</td>
                  <td className="text-right text-red-600 font-medium">{fmt(order.balance)}</td>
                </tr>
              )}
            </tfoot>
          </table>
        ...
        {customer.totalDebt > 0 ? (
          <div className="mt-4 bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-500">You have an outstanding balance of {fmt(customer.totalDebt)}</p>
            </div>
            <p className="ml-auto text-lg font-bold text-red-600 flex-shrink-0">{fmt(customer.totalDebt)}</p>
          </div>
        ) : (
          <div className="mt-4 bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-green-500">No outstanding balance</p>
            </div>
            {customer.wallet > 0 && (
              <p className="ml-auto text-sm font-semibold text-green-600 flex-shrink-0">
                Wallet: {fmt(customer.wallet)}
              </p>
            )}
          </div>
        )}
              ...
              >
                ...
                {tab.count > 0 && (
                  <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${
                    activeTab === tab.id ? 'bg-bge-green text-white' : 'bg-gray-100 text-gray-500'
                  }`}>
                    {tab.count}
                  </span>
                )}
